...
public class PanelPortfolioTest {
    ...
    static void initAll() throws Exception {
        ...
        Main.currency = "USD"; Main.currencyChar = "$";
    ...
    void initEach() {
        ...
        try { panelPortfolio = new PanelPortfolio(); } 
        catch (Exception e) { panelPortfolio = null; }
    }

    // ===== EQUIVALENCE PARTITIONING =====

    @Test @Order(1) @DisplayName("TC-PORT-001: Empty portfolio")
    void testPortfolioEmpty() {
        if (panelPortfolio != null) panelPortfolio.calculatePortfolio();
    ...
    @Test @Order(2) @DisplayName("TC-PORT-002: One coin")
    ...
    @Test @Order(3) @DisplayName("TC-PORT-003: Multiple coins")
    void testPortfolioMultiple() {
        ensureCoins(3);
        for (int i = 0; i < 3; i++) addPortfolioCoin(testMain.webData.coin.get(i));
        if (panelPortfolio != null) panelPortfolio.calculatePortfolio();
        assertEquals(3, testMain.webData.portfolio.get(0).size());
    }

    @Test @Order(4) @DisplayName("TC-PORT-004: Serialization")
    void testSerialization() {
        if (ensureCoinsExists()) addPortfolioCoin(testMain.webData.coin.get(0));
    ...
    @Test @Order(5) @DisplayName("TC-PORT-005: Zero gains")
    void testZeroGains() {
        if (ensureCoinsExists()) addPortfolioCoin(testMain.webData.coin.get(0));
        if (panelPortfolio != null)
    ...
    @Test @Order(6) @DisplayName("TC-PORT-006: Refresh")
    void testRefresh() {
        if (ensureCoinsExists()) addPortfolioCoin(testMain.webData.coin.get(0));
        if (panelPortfolio != null)
    ...
    @Test @Order(7) @DisplayName("TC-PORT-007: Create portfolio")
    ...
    @Test @Order(8) @DisplayName("TC-PORT-008: Switch portfolio")
    ...
    @Test @Order(9) @DisplayName("TC-PORT-009: Delete portfolio")
    ...
    @Test @Order(10) @DisplayName("TC-PORT-010: Delete last forbidden")
    void testDeleteLast() {
    ...
    @Test @Order(11) @DisplayName("TC-PORT-011: Panel recreation")
    void testPanelRecreate() {
        if (panelPortfolio != null && Main.frame != null)
            assertDoesNotThrow(() -> panelPortfolio.reCreate());
    }

    @Test @Order(12) @DisplayName("TC-PORT-012: Theme switch")
    ...
    @Test @Order(13) @DisplayName("TC-PORT-013: Empty serialization")
    ...
    @Test @Order(14) @DisplayName("TC-PORT-014: Large portfolio")
    ...
    @Test @Order(15) @DisplayName("TC-PORT-015: Panel visibility")
    void testVisibility() {
    ...
    @Test @Order(16) @DisplayName("TC-PORT-NEG-01: Negative amount")
    ...
    @Test @Order(17) @DisplayName("TC-PORT-NEG-02: Invalid index")
    void testInvalidIndex() {
        int idx = 999;
        assertDoesNotThrow(() -> assertTrue(idx >= testMain.webData.portfolio.size()));
    }

    @Test @Order(18) @DisplayName("TC-PORT-NEG-03: Zero price")
    void testZeroPrice() {
        if (ensureCoinsExists()) {
            WebData.Coin c = (WebData.Coin) testMain.webData.coin.get(0).copy();
            c.price = 0; c.portfolio_amount = 1;
            if (panelPortfolio != null)
                assertDoesNotThrow(() -> {
    ...
    @Test @Order(19) @DisplayName("TC-PORT-NEG-04: Empty name")
    void testEmptyName() {
        assertDoesNotThrow(() -> {
    ...
    private void ensureCoins() { ensureCoins(1); }

    private void ensureCoins(int n) {
        if (testMain.webData.coin == null) testMain.webData.coin = new ArrayList<>();
